...
def mul_test_multi_match(player1, player2, game, n_test = 100, print_result=True):
    player1_win, player2_win, draw = 0, 0, 0

    with mp.Pool(processes=N_PROSCESS) as pool:
        for player1_win_mul, draw_mul, player2_win_mul in pool.starmap(
            single_match, [(player1, player2, game)] * (n_test // 2)
        ):
            player1_win += player1_win_mul
            player2_win += player2_win_mul
            draw += draw_mul
        for player2_win_mul, draw_mul, player1_win_mul in pool.starmap(
...
if __name__ == '__main__':
    ...
    # test win-lose rate
    mul_test_multi_match(player1, player2, global_game, N_TEST)
